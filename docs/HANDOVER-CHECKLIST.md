...
- E-invoicing (IRN/GST portal) is **built and tested but switched off**. Set `GSP_BASE_URL`, `GSP_CLIENT_ID` and `GSP_CLIENT_SECRET` together to enable it; with any of them missing every invoice stays at `NOT_REQUIRED` and billing is unaffected. Confirm applicability with the CA first — it is mandatory only above the AATO threshold (§1.6). The 24-hour cancellation rule and retry-on-outage are enforced in `src/kernel/einvoice`, which is at 100% branch coverage.
