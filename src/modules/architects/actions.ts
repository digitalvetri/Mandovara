...
// Architect + commission server actions (§5.2, Phase 6b).
//
// Commissions are STAMPED at order-creation time (inside
// createOrderFromQuotation) with the architect's current rate
// captured onto the row — see modules/orders/actions.ts. This
// module owns:
//   - Architect CRUD
//   - recordCommissionPayment (mark paidAt + paymentRef)
//   - cancelCommission (audited undo — e.g. order cancelled)
...
export async function createArchitect(
  ...
  const row = await db.architect.create({
    data: {
      ...
      commissionPct: new Prisma.Decimal(d.commissionPct),
...
export async function updateArchitect(
  ...
  await db.architect.update({
    ...
    data: {
      ...
      ...(commissionPct != null && { commissionPct: new Prisma.Decimal(commissionPct) }),
...
export async function recordCommissionPayment(
  ...
  const ctx = await devContext();
  requirePermission(ctx, "commission.recordPayment");
...
// ── cancelCommission — audited undo ─────────────────────────────
// Used when the underlying order is cancelled or the commission
// was stamped in error. Leaves the row in place with cancelledAt
// + cancelReason so the trail is preserved; queries filter it out
// of "open" totals.
export async function cancelCommission(
  input: unknown,
): Promise<ActionResult<{ commissionId: string }>> {
  const ctx = await devContext();
  requirePermission(ctx, "commission.cancel");
