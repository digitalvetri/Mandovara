  ...
  if (rows.length === 0) {
    return (
      <div className="rounded-[14px] bg-surface border border-rule py-16 text-center">
        <div className="text-[14px] text-text mb-2">No sales orders yet.</div>
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.id} className="border-b border-rule/70 last:border-0 hover:bg-surface-hover transition-colors">
              <Td>
                <Link href={`/orders/${r.id}` as Route} className="text-text hover:text-accent tabular">
                  {r.number}
                </Link>
              </Td>
              <Td>{r.clientName}</Td>
              <Td className="text-text-dim tabular text-[11.5px]">{r.quotationNumber ?? "—"}</Td>
              ...
              <Td className="text-text-dim tabular">{r.deliveryBy ? formatDate(r.deliveryBy) : "—"}</Td>
