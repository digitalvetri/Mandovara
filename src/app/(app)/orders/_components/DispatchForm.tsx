...
// Inline "New dispatch" form under a Sales Order. Enter a qty per pending
// line and submit — creates one Dispatch + DispatchLines, ratchets
// dispatchedQty, and flips the order status if it's now complete.

import { useState, useTransition } from "react";
import { useRouter } from "next/navigation";
import { createDispatch } from "@/modules/orders/actions";
import type { OrderLine } from "@/modules/orders/queries";
...
export function DispatchForm({ orderId, lines }: Props) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [pending, startTransition] = useTransition();
  const [error, setError] = useState<string | null>(null);
  const [dispatchedAt, setDispatchedAt] = useState<string>(iso(new Date()));
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    const linesPayload = pendingLines
      .map((l) => ({ orderLineId: l.id, quantity: Number(qtyByLine[l.id] ?? 0) }))
      .filter((l) => l.quantity > 0);
    if (linesPayload.length === 0) { setError("Enter a quantity to dispatch on at least one line."); return; }
    startTransition(async () => {
      const res = await createDispatch({
        salesOrderId: orderId, dispatchedAt,
        vehicleNumber, transporter, lines: linesPayload,
      });
      if (!res.ok) { setError(res.error ?? "Could not create dispatch"); return; }
      setOpen(false);
      setQtyByLine({});
      setVehicleNumber(""); setTransporter("");
      router.refresh();
    });
  }
    ...
    return (
      <div className="rounded-[14px] bg-surface border border-rule p-4 flex items-center justify-between">
        <div className="text-[12.5px] text-text-dim">
          {pendingLines.length} line{pendingLines.length === 1 ? "" : "s"} pending dispatch.
        </div>
        <button
          type="button"
  ...
  return (
    <form onSubmit={onSubmit} className="rounded-[14px] bg-surface border border-rule p-5">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
        <Field label="Dispatched on" required>
          <input type="date" value={dispatchedAt}
                 onChange={(e) => setDispatchedAt(e.target.value)}
                 className={`${fieldCls} tabular`} />
        </Field>
        <Field label="Vehicle number">
          <input value={vehicleNumber} onChange={(e) => setVehicleNumber(e.target.value)}
                 className={`${fieldCls} tabular uppercase`} placeholder="e.g. TN 39 AB 1234" />
        </Field>
        <Field label="Transporter">
          <input value={transporter} onChange={(e) => setTransporter(e.target.value)}
                 className={fieldCls} placeholder="e.g. VRL Logistics" />
        </Field>
      </div>
        ...
        <table className="w-full text-[12.5px]">
          <thead>
            <tr className="border-b border-rule text-[10.5px] uppercase tracking-[0.14em] text-text-dim">
              ...
              <Th align="right">Already dispatched</Th>
              <Th align="right">Pending</Th>
              <Th align="right" width={140}>Dispatch now</Th>
            </tr>
          </thead>
          <tbody>
            {pendingLines.map((l) => (
              <tr key={l.id} className="border-b border-rule/60 last:border-0">
                ...
                <Td align="right"><span className="tabular text-text-dim">{l.dispatchedQty}</span></Td>
