...
// The new-expense form itself — head, amount, note, date, how it was
// paid, and GST behind a toggle.
//
// Split out of NewExpenseForm.tsx on 2026-09-04 when the payment-mode
// row pushed that file past the §10 300-line ceiling. The seam is the
// natural one: NewExpenseForm owns *where* the trigger and the panel
// appear in the tab, this owns *what the panel contains*.

import { useState, useTransition } from "react";
import { useRouter } from "next/navigation";
import { Loader2, IndianRupee, X, ChevronDown } from "lucide-react";
import { createExpense } from "@/modules/expenses/actions";
import { GENERAL_EXPENSE_HEADS, type ExpensePaymentMode } from "@/modules/expenses/schema";
import { ExpenseModePicker } from "./_expense-mode";
import {
  GstFields, GstPreview, expenseFieldCls, expenseLabelCls,
} from "./_expense-gst";
import { safePaise, iso } from "./_receipt-primitives";

export function NewExpenseSheet({ onClose }: { onClose: () => void }) {
  ...
  // No default. Pre-selecting "Cash" would make it the answer on every
  // expense nobody read the field on, which is exactly the kind of
  // plausible-but-wrong data the reconciliation is meant to catch.
  const [paymentMode, setPaymentMode] = useState<ExpensePaymentMode | null>(null);
  ...
  const canSubmit =
    !!finalHead && description.trim().length >= 3 && totalPaise > 0n &&
    paymentMode !== null && !pending;
  ...
  return (
    <div className="mb-5 overflow-hidden rounded-[14px] border border-gold/40 bg-surface">
      ...
      <form onSubmit={onSubmit} className="grid grid-cols-1 gap-x-4 gap-y-3.5 p-5 sm:grid-cols-12">
        ...
        {/* How it was paid. Sits with "When?" because the two are the
            same question about the same event — what happened, and how.
            Above the GST toggle, which is the optional half. */}
        <ExpenseModePicker
          mode={paymentMode}
        ...
        <div className="mt-1 flex flex-col gap-3 border-t border-rule pt-4 sm:col-span-12 sm:flex-row sm:items-center sm:justify-between">
          <div className="text-[11px] text-text-dim">
            {canSubmit
              ? "Ready to save."
              : paymentMode === null && !!finalHead && totalPaise > 0n
              ? "Choose how it was paid."
              : "Pick a head, add a short note, an amount and how it was paid."}
          </div>
