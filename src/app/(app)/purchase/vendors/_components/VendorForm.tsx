...
import { createVendorSchema, type CreateVendorInput } from "@/modules/vendors/schema";
...
export function VendorForm({ mode, initial }: Props) {
  ...
  const {
    ...
    defaultValues: {
      ...
      brandIds:         initial?.brandIds ?? [] as string[],
  ...
  function onSubmit(values: CreateVendorInput) {
    ...
    startTransition(async () => {
      ...
      const result = mode === "edit" ? await updateVendor(payload) : await createVendor(payload);
      if (!result.ok) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="rounded-[14px] bg-surface border border-rule p-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
        ...
        <Field label="Payment terms (days)" error={errors.paymentTermsDays?.message} required>
          <input
